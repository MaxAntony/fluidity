...
export const Searchbar = () => {
  ...
  const redirectToSearch = (query: string) => {
    if (searchSettings.fastForward[query]) window.location.href = searchSettings.fastForward[query];
    else {
      // for compatibility with old engine urls before fluidity 0.5.0
      if (!engine.includes(queryToken)) window.location.href = 'https://' + engine + '?q=' + query;
  ...
  return (
    <StyledSearchbarContainer>
      ...
      <StyledSearchbar
        placeholder={"let's conquer the world!"}
        type='input'
